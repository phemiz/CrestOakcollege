...
const FeesTable: React.FC<{ title: string; data: FeeStructureItem[]; showTotal?: boolean }> = ({ title, data, showTotal }) => {
    const { theme } = useTheme();
    const borderColor = theme.name === 'light' ? 'border-gray-200' : 'border-white/10';
    const headerBg = theme.name === 'light' ? 'bg-gray-100' : 'bg-white/5';
    const rowStriped = theme.name === 'light' ? 'even:bg-gray-50' : 'even:bg-white/5';

    // Helper to extract number from currency string for totaling
    ...
    return (
        <div className={`mb-8 ${theme.card.background} rounded-lg shadow border ${theme.card.border} overflow-hidden`}>
            <div className={`p-4 border-b ${borderColor}`}>
                <h3 className={`text-lg font-bold ${theme.accent} uppercase`}>{title}</h3>
            </div>
            <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                    <thead className={headerBg}>
                        <tr>
                            <th className={`p-3 font-semibold ${theme.text}`}>Item</th>
                            <th className={`p-3 font-semibold ${theme.text}`}>Amount</th>
                            <th className={`p-3 font-semibold ${theme.text}`}>Note</th>
                        </tr>
                    </thead>
                    <tbody>
                        {data.map((row) => (
                            <tr key={row.id} className={`${rowStriped} border-b ${borderColor} last:border-0`}>
                                <td className={`p-3 ${theme.text}`}>{row.item}</td>
                                <td className={`p-3 ${theme.text}`}>{row.amount}</td>
                                <td className={`p-3 ${theme.textMuted}`}>{row.note}</td>
                            </tr>
                        ))}
                        {showTotal && (
                            <tr className={`font-bold border-t-2 ${borderColor} ${theme.name === 'light' ? 'bg-blue-50' : 'bg-blue-900/20'}`}>
                                <td className={`p-3 ${theme.text}`}>TOTAL</td>
                                <td className={`p-3 ${theme.text}`}>₦{totalAmount.toLocaleString()}</td>
                                <td className={`p-3 ${theme.textMuted}`}>(Per Semester)</td>
                            </tr>
                        )}
                    </tbody>
                </table>
...
const AdmissionsPage: React.FC = () => {
    ...
    const { data: feeStructure, loading: feesLoading } = useApi<FeeStructureItem[]>('/api/fee-structure');

    const generalFees = feeStructure?.filter(f => f.category === 'General') || [];
    const adminCharges = feeStructure?.filter(f => f.category === 'Administrative') || [];
  ...
  return (
    <PageWrapper
      title="Admissions"
      subtitle="Begin your academic journey at CrestOAK College. We're excited to welcome you."
    >
      <div className="max-w-4xl mx-auto grid md:grid-cols-3 gap-12">
        ...
        <div className="md:col-span-2 space-y-12">
          <section>
            ...
            <div>
                ...
                <AdmissionStep step={2} title="Entrance Examination">
                    Eligible candidates will be invited to take the CrestOAK College entrance examination.
                </AdmissionStep>
          ...
          <section>
            <h2 className={`text-3xl font-bold ${theme.text} mb-6`}>Our School Fees Structure</h2>
            <p className={`mb-6 ${theme.textMuted}`}>The fees structure below is applicable to all new students.</p>
            
            {feesLoading ? <TableSkeleton /> : (
                <>
                    <FeesTable title="School Fees Structure" data={generalFees} />
                    <FeesTable title="Administrative Charges" data={adminCharges} showTotal={true} />
                </>
            )}
          </section>
