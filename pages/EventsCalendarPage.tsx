...
import { useApi, rsvpToEvent } from '../hooks/useApi';
...
const generateIcsFile = (event: CalendarEvent) => {
    ...
    const icsContent = [
        'BEGIN:VCALENDAR',
        'VERSION:2.0',
        'PRODID:-//Crestview College//Event Calendar//EN',
        'BEGIN:VEVENT',
        `UID:${event.id}@crestview.college`,
                                        ...
                                        <div key={event.id}>
                                            ...
                                            {event.category !== 'Holiday' && (
                                                <div className="flex items-center justify-between mt-1">
                                                    ...
                                                    {event.rsvps > 0 && (
                                                        <span className="flex items-center text-xs text-gray-500" title={`${event.rsvps} people going`}>
                                ...
                                <li key={event.id} className={`p-3 rounded-md flex items-center justify-between ${theme.name === 'light' ? 'bg-gray-100' : 'bg-white/5'}`}>
                                    ...
                                    {event.category !== 'Holiday' && (
                                        <div className="flex items-center gap-4 ml-4 flex-shrink-0">
                                            {event.rsvps > 0 && (
                                                <span className="flex items-center text-sm text-gray-500" title={`${event.rsvps} people going`}>
...
const EventsCalendarPage: React.FC = () => {
    ...
    const RSVP_STORAGE_KEY = 'crestview_rsvped_events';
    ...
    const handleRsvp = useCallback(async (eventId: string, e: React.MouseEvent) => {
        ...
        if (result.success && result.newRsvpCount !== undefined) {
            setEvents(prevEvents => 
                prevEvents?.map(ev => ev.id === eventId ? { ...ev, rsvps: result.newRsvpCount! } : ev) || null
            );
