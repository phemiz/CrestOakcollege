        ...
        <div className={`${theme.card.background} ${theme.card.rounded} ${theme.card.shadow} ${theme.card.border} ${theme.card.transition} ${theme.card.hover} overflow-hidden flex flex-col`}>
            <div className="p-6 flex-grow">
             ...
             <div className={`${theme.name === 'light' ? 'bg-gray-50 border-gray-100' : 'bg-white/5 border-white/10'} p-4 mt-auto border-t`}>
                <span className={`font-semibold ${theme.text}`}>Read More</span>
            </div>
        </div>
    );
};
...
const NewsAndEventsPage: React.FC = () => {
    ...
    return (
        <PageWrapper
            title="College News"
            subtitle="Stay updated with the latest announcements and happenings at Crestview College."
        >
            {loading && (
                <div className="text-center">
                    ...
                    <p className={`mt-4 ${theme.textMuted}`}>Loading News...</p>
                </div>
            )}
            {error && <p className="text-center text-red-500">Error loading news: {error}</p>}
