...
const BlogPostPage: React.FC = () => {
    ...
    useEffect(() => {
        if (post) {
            document.title = `${post.title} - CrestOAK College`;
            const metaDescription = document.querySelector('meta[name="description"]');
            if (metaDescription) {
                metaDescription.setAttribute('content', post.excerpt);
            }
        } else if (!loading) {
            document.title = 'Post Not Found - CrestOAK College';
        }
    }, [post, loading]);
    ...
    const blogPostSchema = {
        ...
        "publisher": {
            ...
            "logo": {
                "@type": "ImageObject",
                "url": "https://crestoak.edu.ng/logo.png"
            }
        },
    };
                ...
                <div className="max-w-4xl mx-auto">
                    <div className="text-center mb-8">
                        ...
                        <p className={`${theme.textMuted} text-sm`}>
                            Published on {new Date(post.createdAt).toLocaleDateString()}</p>
                    </div>
