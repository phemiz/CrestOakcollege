...
const EnrollmentModal: React.FC<EnrollmentModalProps> = ({ isOpen, onClose, student, allCourses, departments, onSubmit }) => {
    ...
    const { data: enrolledCourseIds, loading: enrollmentsLoading, refetch } = useApi<string[]>('/api/student-enrollments', isOpen ? student.id : undefined);
    ...
    useEffect(() => {
        if (isOpen) {
            // refetch is automatically called by useApi when `id` changes,
