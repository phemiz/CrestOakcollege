...
export default function ClientPortalShell({ children, user, announcements }: ClientPortalShellProps) {
  ...
  const { isAuthorized, isChecking } = useRequireAuth({ allowedRoles: ["STUDENT"], gateway: "portal" });
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="bg-white border border-slate-200 p-8 rounded-3xl shadow-xl max-w-sm w-full text-center space-y-4">
          ...
          <div>
            <h3 className="font-display font-extrabold text-slate-900 text-base">Verifying Student Portal</h3>
            <p className="text-slate-500 text-xs mt-1">Redirecting to Student Portal Login...</p>
          </div>
        </div>
      </div>
    );
