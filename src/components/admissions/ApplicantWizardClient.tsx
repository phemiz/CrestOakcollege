...
export default function ApplicantWizardClient({
  ...
  const handleFinalSubmit = async (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      // 1. Save draft application first to ensure Application model instance is established
  ...
  return (
    <div className="max-w-2xl mx-auto bg-slate-950 border border-slate-800 rounded-3xl p-6 md:p-8 shadow-2xl relative overflow-hidden text-xs text-slate-300 font-semibold">
      {/* Visual step indicator */}
      <div className="h-1 bg-slate-900 absolute top-0 left-0 right-0">
        <div
      ...
      <div className="flex justify-between items-center pb-4 border-b border-slate-800 mb-6">
        ...
        <div className="flex gap-2">
          {saveDraftSuccess && (
            <span className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/25 px-3 py-1.5 rounded-xl font-bold uppercase tracking-wider text-[10px] flex items-center gap-1.5">
          ...
          <button
            ...
            className="bg-slate-900 border border-slate-800 hover:bg-slate-850 hover:text-white px-3.5 py-1.5 rounded-xl transition-all cursor-pointer flex items-center gap-1 text-[10px] uppercase font-bold"
          >
      ...
      {errorStatus && (
        <div className="mb-6 flex items-start gap-3 bg-red-500/10 border border-red-500/30 text-red-200 p-4 rounded-xl text-sm transition-all">
      ...
      {step === 1 && (
        <div className="space-y-5">
          ...
          <div className="flex flex-col gap-2">
            <label className="text-[10px] text-slate-400 uppercase tracking-wider font-bold">Study Level *</label>
            <select
              name="level"
          ...
          <div className="flex flex-col gap-2">
            <label className="text-[10px] text-slate-400 uppercase tracking-wider font-bold">Select Programme *</label>
            <select
              name="programmeId"
          ...
          <div className="flex flex-col gap-2">
            <label className="text-[10px] text-slate-400 uppercase tracking-wider font-bold">Gender *</label>
            <select
              name="gender"
      ...
      {step === 2 && (
        <div className="space-y-5">
          ...
          {formData.level === "undergraduate" ? (
            <div className="flex flex-col gap-2">
              <label className="text-[10px] text-slate-400 uppercase tracking-wider font-bold">UTME JAMB Score *</label>
              <input
                type="number"
                name="jambScore"
                ...
                onChange={handleChange}
                placeholder="e.g. 215"
                className="p-3 bg-slate-900 border border-slate-850 rounded-xl text-slate-200 focus:outline-none focus:border-indigo-500 font-mono font-bold"
              />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="flex flex-col gap-2">
                <label className="text-[10px] text-slate-400 uppercase tracking-wider font-bold">Graduating Institution *</label>
                <input
                  type="text"
                  ...
                  className="p-3 bg-slate-900 border border-slate-850 rounded-xl text-slate-200 focus:outline-none focus:border-indigo-500"
                />
              ...
              <div className="flex flex-col gap-2">
                <label className="text-[10px] text-slate-400 uppercase tracking-wider font-bold">Graduating Class *</label>
                <select
          ...
          <div className="flex flex-col gap-2">
            <label className="text-[10px] text-slate-400 uppercase tracking-wider font-bold">O&apos;Level Core Credit Count *</label>
            <select
              name="olevelCredits"
        ...
        <div className="space-y-5">
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-between items-center bg-slate-900 p-4 rounded-xl border border-slate-850">
              <div>
                <p className="font-bold text-slate-200">O&apos;Level Result Slip *</p>
              ...
              <button
                ...
                className="bg-slate-950 border border-slate-800 hover:bg-slate-850 text-slate-300 font-bold py-1.5 px-3 rounded-lg flex items-center gap-1 cursor-pointer text-[10px]"
              >
            ...
            <div className="flex justify-between items-center bg-slate-900 p-4 rounded-xl border border-slate-850">
              <div>
                <p className="font-bold text-slate-200">Passport Photograph *</p>
                <p className="text-[10px] text-slate-500 mt-0.5 truncate max-w-[200px]">
      ...
      {step === 4 && (
        <form onSubmit={handleFinalSubmit} className="space-y-5">
          ...
          <div className="bg-slate-900 p-4 rounded-xl border border-slate-850 space-y-4 text-slate-350">
            <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <strong className="text-slate-200 text-xs block mt-0.5 uppercase">{formData.level}</strong>
              </div>
              <div>
                <span className="text-[10px] text-slate-500 block uppercase">O&apos;Level Core Credits</span>
                <strong className="text-slate-200 text-xs block mt-0.5">{formData.olevelCredits} Credits</strong>
              </div>
            </div>
            {formData.level === "undergraduate" ? (
              <div>
                <span className="text-[10px] text-slate-500 block uppercase">UTME JAMB Score</span>
                <strong className="text-slate-200 text-xs block mt-0.5">{formData.jambScore} Marks</strong>
              </div>
            ) : (
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <span className="text-[10px] text-slate-500 block uppercase">Degree Institution</span>
                  <strong className="text-slate-200 text-xs block mt-0.5 truncate">{formData.firstDegreeInstitution}</strong>
                </div>
                <div>
                  <span className="text-[10px] text-slate-500 block uppercase">Graduation Class</span>
                  <strong className="text-slate-200 text-xs block mt-0.5 uppercase">{formData.firstDegreeClass}</strong>
                </div>
              </div>
            )}
            <div className="pt-2 border-t border-slate-850 flex items-center gap-2 text-[10px] text-slate-500 font-semibold italic">
          ...
          <button
            type="submit"
            disabled={isPending}
            className="w-full bg-red-600 hover:bg-red-700 disabled:opacity-50 text-white font-display font-bold py-3 rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1.5 shadow-md shadow-red-950/20 text-xs mt-2"
          >
      ...
      <div className="flex justify-between items-center pt-5 border-t border-slate-800 mt-6">
        <button
          ...
          disabled={step === 1}
          onClick={handleBack}
          className="bg-slate-900 border border-slate-850 hover:bg-slate-850 text-slate-200 font-display font-bold py-2.5 px-4 rounded-xl text-[10px] uppercase tracking-wider disabled:opacity-50 disabled:cursor-not-allowed transition-colors cursor-pointer flex items-center gap-1.5"
        >
        ...
        {step < 4 && (
          <button
            type="button"
            onClick={handleNext}
            className="bg-indigo-600 hover:bg-indigo-700 text-white font-display font-bold py-2.5 px-5 rounded-xl text-[10px] uppercase tracking-wider transition-colors cursor-pointer flex items-center gap-1.5 shadow-md shadow-indigo-950/15"
          >
            <span>Next Step</span>
