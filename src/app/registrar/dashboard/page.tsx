  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col justify-center items-center bg-slate-950 text-white">
        ...
        <p className="text-sm font-medium text-slate-300">Loading University Registrar Workspace...</p>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 font-sans flex flex-col md:flex-row">
        ...
        {/* SIDEBAR NAVIGATION */}
        <aside className="w-full md:w-72 bg-slate-900 border-r border-slate-800 flex flex-col justify-between shrink-0">
          <div className="p-6 space-y-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-sm font-bold text-white tracking-wide">University Registrar</h2>
                <p className="text-xs text-indigo-400 font-medium">CrestOak Governance</p>
              </div>
            </div>
            ...
            <div className="p-3.5 rounded-2xl bg-slate-950/80 border border-slate-800 flex items-center gap-3">
              ...
              <div className="overflow-hidden">
                <p className="text-xs font-bold text-white truncate">{currentUser?.name || "University Registrar"}</p>
                <p className="text-[11px] text-slate-400 truncate">{currentUser?.email || "registrar@crestoakcollege.com.ng"}</p>
              </div>
            </div>
          ...
          <div className="p-6 border-t border-slate-800/80 space-y-3">
            <div className="p-3 rounded-xl bg-indigo-950/40 border border-indigo-500/20 text-xs text-indigo-300 space-y-1">
              ...
              <p className="text-[11px] text-slate-400">Current Session: 2026/2027</p>
            </div>
        ...
        <main className="flex-grow p-4 sm:p-6 md:p-8 space-y-6 overflow-x-hidden">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="p-5 rounded-2xl bg-slate-900 border border-slate-800 space-y-2">
              ...
              <p className="text-xs text-indigo-400 font-medium">Awaiting Registrar Seal</p>
            </div>
            ...
            <div className="p-5 rounded-2xl bg-slate-900 border border-slate-800 space-y-2">
              ...
              <p className="text-2xl font-extrabold text-white">
                {courses.length} <span className="text-xs text-slate-400 font-normal">({courses.filter(c => c.status === "Pending Review").length} pending)</span>
              </p>
          ...
          {activeTab === "records" && (
            <div className="space-y-6">
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
                <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between">
                  <h4 className="text-sm font-bold text-white">Transcript Applications & Status Roster</h4>
                  <span className="text-xs text-indigo-400 font-semibold">{students.length} Records Enrolled</span>
                </div>
                        ...
                        .map((st) => (
                          <tr key={st.id} className="hover:bg-slate-800/40 transition-colors">
                            <td className="px-6 py-4">
                              ...
                              <p className="text-[11px] font-mono text-slate-400">{st.matricNo}</p>
                            </td>
                            <td className="px-6 py-4 text-slate-300">{st.department}</td>
                            <td className="px-6 py-4">
                              <span className="font-mono text-indigo-300 font-bold text-sm">{st.cgpa.toFixed(2)}</span>
                              <span className="text-[11px] text-slate-400 block">{st.totalUnits} Units</span>
                            </td>
                            <td className="px-6 py-4">
                              <span
                                className={`inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-extrabold ${
                            ...
                            <td className="px-6 py-4">
                              {st.transcriptStatus === "Pending" && (
                                <div>
                                  ...
                                  <p className="text-[10px] text-slate-400 truncate max-w-[150px]">{st.destination}</p>
                                </div>
                              )}
                              {st.transcriptStatus === "Approved" && (
                                <span className="inline-flex items-center gap-1 text-indigo-400 text-[11px] font-bold">
                            ...
                            <td className="px-6 py-4 text-right space-x-2">
                              <button
                                ...
                                className="px-2.5 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-lg text-[11px] font-bold transition-all"
                              >
                                Verify CGPA
                              </button>
                              ...
                              <button
                                ...
                                className="px-2.5 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg text-[11px] font-bold transition-all"
                              >
                                Change Status
                              </button>
                            </td>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
                  <h4 className="text-sm font-bold text-white flex items-center justify-between border-b border-slate-800 pb-3">
                    ...
                    <span className="text-xs text-slate-400">2026/2027 Semester</span>
                  </h4>
                        ...
                        <div key={course.id} className="p-4 rounded-xl bg-slate-950 border border-slate-800 space-y-3">
                          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                            <div>
                              <div className="flex items-center gap-2">
                                <span className="font-mono text-xs font-extrabold text-indigo-400 bg-indigo-950/60 px-2 py-0.5 rounded border border-indigo-500/30">
                                  {course.code}
                                </span>
                          ...
                          <div className="space-y-1.5">
                            <div className="flex items-center justify-between text-xs font-medium">
                              <span className="text-slate-400">Enrollment: <strong className="text-white">{course.currentEnrollment}</strong> / {course.capacityCap}</span>
                              <span className={`font-mono font-bold ${pct >= 95 ? "text-red-400" : "text-indigo-300"}`}>{pct}% Full</span>
                            </div>
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
                  <h4 className="text-sm font-bold text-white flex items-center justify-between border-b border-slate-800 pb-3">
                    ...
                    <span className="text-xs text-indigo-400">{overrides.length} Issued</span>
                  </h4>

                  <div className="space-y-3">
                    {overrides.map((ov) => (
                      <div key={ov.id} className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 text-xs space-y-2">
                        <div className="flex items-center justify-between">
                          <span className="font-mono text-indigo-400 font-bold">{ov.courseCode}</span>
                          <span className="text-slate-500 text-[10px]">{ov.date}</span>
                        </div>
                        <p className="font-bold text-white">{ov.studentName}</p>
                        <p className="text-slate-400 text-[11px] leading-relaxed">{ov.reason}</p>
                        <p className="text-[10px] text-slate-500">Issued by: <strong className="text-slate-300">{ov.issuedBy}</strong></p>
                      </div>
                    ))}
                  </div>
                </div>
                      ...
                      {degreeAudits.map((audit) => (
                        <tr key={audit.id} className="hover:bg-slate-800/40 transition-colors">
                          <td className="px-6 py-4">
                            ...
                            <p className="text-[11px] font-mono text-slate-400">{audit.matricNo}</p>
                          </td>
                          <td className="px-6 py-4 text-slate-300">{audit.degree}</td>
                          <td className="px-6 py-4">
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
                ...
                <form onSubmit={handleVerifyDiploma} className="flex flex-col sm:flex-row gap-3 max-w-xl">
                  <input
                    ...
                    className="flex-grow px-4 py-2.5 bg-slate-950 border border-slate-700 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500"
                  />
                  <button
                    type="submit"
                  ...
                  <div className={`p-4 rounded-xl border text-xs space-y-2 mt-4 ${
                    ...
                    {diplomaResult.valid ? (
                      <div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-slate-300 font-mono text-[11px] pt-2">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
                  ...
                  <div className="space-y-3 text-xs">
                    ...
                    <div className="flex justify-between py-1.5 border-b border-slate-800/60">
                      ...
                      <strong className="text-indigo-300 font-mono">{calendarSettings.regStartDate}</strong>
                    </div>
                    <div className="flex justify-between py-1.5 border-b border-slate-800/60">
                      <span className="text-slate-400">Normal Registration Deadline:</span>
                      <strong className="text-amber-300 font-mono">{calendarSettings.regEndDate}</strong>
                    </div>
                    <div className="flex justify-between py-1.5 border-b border-slate-800/60">
                      <span className="text-slate-400">Late Registration Cut-off:</span>
                      <strong className="text-red-400 font-mono">{calendarSettings.lateRegEndDate}</strong>
                    </div>
                    <div className="flex justify-between py-1.5">
                      <span className="text-slate-400">Final Examinations Period:</span>
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
                  ...
                  <div className="space-y-3">
                    {gradeLocks.map((g) => (
                      <div key={g.departmentId} className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 flex items-center justify-between gap-3 text-xs">
                        <div>
                          <p className="font-bold text-white">{g.departmentName}</p>
                          <p className="text-[11px] text-slate-400">{g.faculty}</p>
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 max-w-md w-full space-y-4 animate-in fade-in zoom-in-95">
            ...
            <div className="text-xs space-y-2">
              <p className="text-slate-300">Student: <strong className="text-white">{statusModalStudent.name}</strong></p>
              <p className="text-slate-400 font-mono">Matric: {statusModalStudent.matricNo}</p>
              <p className="text-slate-400">Current Status: <strong className="text-indigo-400">{statusModalStudent.status}</strong></p>
            </div>

            <div className="space-y-2 pt-2">
              <p className="text-xs font-bold text-slate-300">Select New Status:</p>
      ...
      {newOverrideModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <form onSubmit={handleAddOverride} className="bg-slate-900 border border-slate-800 rounded-3xl p-6 max-w-md w-full space-y-4 animate-in fade-in zoom-in-95">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <h4 className="text-sm font-bold text-white">Issue Prerequisite Waiver</h4>
              <button type="button" onClick={() => setNewOverrideModal(false)} className="text-slate-400 hover:text-white">
                <XCircle className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="space-y-3 text-xs">
              <div>
                <label className="block text-slate-300 font-bold mb-1">Student Matric Number</label>
                <input
                  name="matricNo"
                  type="text"
                  required
              ...
              <div>
                <label className="block text-slate-300 font-bold mb-1">Target Course Code</label>
                <input
                  name="courseCode"
                  type="text"
                  required
              ...
              <div>
                <label className="block text-slate-300 font-bold mb-1">Waiver Reason / Board Approval</label>
                <textarea
                  name="reason"
                  required
